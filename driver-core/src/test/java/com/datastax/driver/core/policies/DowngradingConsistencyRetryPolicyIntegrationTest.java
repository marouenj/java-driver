        ...
        try {
            ...
            try {
                ...
                assertThat(e.getErrors().values())
                        .hasOnlyElementsOfType(DriverException.class)
                        .extractingResultOf("getMessage")
                        .containsOnlyOnce(
                                String.format("[%s:9042]: Timed out waiting for server response", host1.getAddress()),
                                String.format("[%s:9042]: Timed out waiting for server response", host2.getAddress()),
                                String.format("[%s:9042]: Timed out waiting for server response", host3.getAddress())
                        );
            }
